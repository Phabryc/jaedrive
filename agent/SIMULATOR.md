...
5. **Directory Temporanea TEMP/TMP**: `sdkmanager` scarica i file `.zip` intermedi nella directory `%TEMP%` di sistema. Se l'unità `C:\` ha spazio limitato, ricordarsi di impostare temporaneamente `$env:TEMP` e `$env:TMP` sull'unità target ad alta capienza prima di invocare `sdkmanager`.
