...
5. **Directory Temporanea Java (JAVA_OPTS / TEMP)**: `sdkmanager` è un'applicazione Java e decompone i pacchetti temporanei in `java.io.tmpdir`. Se l'unità `C:\` ha spazio limitato, impostare `$env:JAVA_OPTS = "-Djava.io.tmpdir=D:\Temp"` oltre a `$env:TEMP` e `$env:TMP` per costringere Java ed `sdkmanager` ad utilizzare l'unità secondaria per tutti i file temporanei.
