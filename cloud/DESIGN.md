...
6. Car app's next poll to `/api/device/pairing/status/{pairingRequestId}` sees `status: claimed` and receives the plaintext device token **once** (never returned again).
7. **Handshake confirmation (2026-08-06, see `agent_log.md`)**: the token alone is not treated as proof the app actually finished pairing — a dismissed dialog, killed process, or crash right after step 6 would otherwise leave a `vehicles`/`devices` row created server-side that the app itself never learned about (stuck forever as "added but not synced"). So the app does **not** persist the token/paired state yet at step 6 — it first calls `PATCH /api/device/vehicle` with the brand/model/powertrain already chosen during onboarding, using the token directly (not yet saved to `SharedPreferences`). Only once that call succeeds does the app save the token and show itself as paired; only then does the server mark `devices.confirmed_at`. If the call fails, the app discards the token and shows a pairing error — the orphaned `devices`/`vehicles` rows get swept up by `cron/pairingCleanup.ts` (below).
8. **Web-side wait**: `POST /api/user/pairing/claim` now also returns `deviceId`. The web `/pair` page does not navigate to the vehicle immediately after claim — it polls `GET /api/user/devices/{deviceId}/confirm-status` (`{ confirmed: boolean }`) every 2s until `confirmed: true` (then navigates) or a 404 (device was cleaned up — show a clear failure message and ask the user to generate a new code from the car) or a 35s client-side timeout (small margin over the server's 30s grace period).

**Cleanup (`cron/pairingCleanup.ts`)**: every 15s, sweeps `devices` rows where `confirmed_at IS NULL` and older than 30s, deletes them, and additionally deletes the parent `vehicles` row if — after removing that device — it has zero other devices and zero trips (i.e. the vehicle add was genuinely never confirmed by any client, not a legitimate re-pair or an already-populated vehicle). This is what makes step 7 safe: a pairing that never completes the handshake cleans itself up instead of leaving a permanent half-added vehicle.
