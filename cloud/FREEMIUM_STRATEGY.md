...
### 4.8 Bugfix critico: feature PREMIUM utilizzabili gratis dopo una disassociazione (2026-08-05)
Segnalato dall'utente durante un test reale: **associare l'auto una volta con un abbonamento qualsiasi, accendere i 3 switch PREMIUM (barra di stato/popup rigenerazione/popup rifornimento) e poi disassociare** lasciava le funzioni popup rigenerazione/rifornimento **pienamente funzionanti per sempre**, senza alcuna sottoscrizione attiva - una scappatoia che avrebbe permesso di ottenere le funzioni PREMIUM gratis ripetendo associazione→attivazione→disassociazione all'infinito. Causa: `Prefs.clearCloudPairing()` rimuoveva solo i dati di sottoscrizione, mai riportava a `false` i 3 switch stessi - e i due popup si basavano *solo* su quello switch, senza mai altro ricontrollo. Analizzando il problema è emerso un secondo bug, più subdolo, dietro al primo: se l'auto veniva disassociata dal **sito** (non dal bottone RIMUOVI dell'app), l'unica chiamata di rete che gira regolarmente ad app aperta e inattiva (`fetchOwnerProfile()`, ogni 5 minuti) ignorava del tutto l'errore `409 "Device is not paired to a vehicle"` restituito dal server - lo stato locale (token, abbonamento "attivo") restava quindi valido all'infinito, e l'app non si accorgeva MAI della disassociazione reale finché non veniva chiusa e riaperta (dove nemmeno lì c'era un controllo dedicato).

Corretto su tre livelli:
1. `Prefs.clearCloudPairing()` ora forza sempre i 3 switch a `false` (nessun backup/ripristino qui, a differenza della scadenza temporanea di abbonamento §4.2 - una disassociazione è un reset deliberato, non una sospensione momentanea).
2. Popup rigenerazione/rifornimento ricontrollano `isSubscriptionActive()` **al momento stesso** in cui scattano, non solo lo switch salvato - difesa in profondità, cosi un bug simile in futuro non riapra la stessa scappatoia (la barra di stato lo faceva già, era l'unica delle 3 al sicuro).
3. `fetchOwnerProfile()` ora gestisce il 409 esattamente come già faceva `SyncWorker` per heartbeat/upload: disassocia subito in locale e mostra l'avviso "auto disassociata da remoto" già esistente nell'app, invece di ignorare l'errore in silenzio.
